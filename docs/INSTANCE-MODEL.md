...
为减少重复安装，四个 worktree 的 `src\node_modules` 可以是指向主项目依赖目录的 Junction；这只共享可重建的依赖，不共享源码、Electron `userData`、GPT 登录态、运行目录或任务数据。依赖升级应先在主项目验证，再统一更新，避免把依赖变更误认为某个账号的代码变更。
