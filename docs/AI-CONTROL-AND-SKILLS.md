...
## 十三、全权自主操作与 20 分钟自愈执行标准（2026-09-02 授权生效）

### 13.1 用户全权授权范围
1. **全面操作权**：允许 AI 通过 API、CDP、Computer Use、MCP、命令行、脚本及状态机对整个软件进行全权自主操控；
2. **源码与架构进化权**：允许 AI 自主诊断缺陷、修改代码、更新机制、优化配置与启动脚本；
3. **自主决策与推进权**：无需人工微调或手动点按，AI 全权自主判断当前卡点、执行自愈并推进生产队列。

### 13.2 20 分钟周期常驻巡检标准
1. **进程与端口健康**：巡检 A/B/C/D 各实例端口（4331-4334、9431-9434）及服务存活，遇孤立占锁自动秒级自愈；
2. **任务卡点主动解除**：
   - 计划确认卡点：自动监听 GPT 输出并秒级回复 `1`；
   - 网页假死/超时卡点：自动执行弹性恢复并保持检查点不丢素材；
   - 队列停滞：自动派发 `gpt.continue` 指令推进；
3. **交付产出核验**：确保 `成品库（GPT+本地脚本制作）` 持续接收并落盘合规的图片与双平台文案。
