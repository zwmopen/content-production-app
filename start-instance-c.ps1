...
$env:CONTENT_INSTANCE_LABEL = "实例 C · 账号3"
$env:PORT = "4333"
...
for ($attempt = 0; $attempt -lt 40; $attempt += 1) {
  ...
  try { $ready = $client.ConnectAsync("127.0.0.1", 4333).Wait(300) } catch { $ready = $false } finally { $client.Dispose() }
  if ($ready) { break }
}
if (-not $ready) { throw "内容生产服务未能监听 4333，详见 $serverErr" }
$exitCode = 0
try {
