...
- 发布文案候选统一由 `lib/caption-file-policy.js` 判定：优先 `文案.txt`、`小红书文案.txt`、`小红书发布文案.txt`、`抖音文案.txt` 等发布文件；`会话追踪`、`生产对话轨迹`、`生产记录`、`生成提示词`、`质量报告`、`作品标签`、`标签`、`元数据`、`manifest` 和日志类 TXT/MD 只作元数据，永不作为发布文案。只有元数据或多个未知 TXT 时会明确返回“无可发布文案/歧义”。
