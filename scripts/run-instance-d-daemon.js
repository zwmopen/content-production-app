...
const env = {
  ...process.env,
  CONTENT_INSTANCE_ID: 'D',
  CONTENT_INSTANCE_LABEL: '实例 D · account-4',
  PORT: '4334',
  TB_REMOTE_DEBUGGING_PORT: '9434',
  CONTENT_HTTP_PROXY: 'http://127.0.0.1:7897',
  TEAMBUILDING_DASHBOARD_RUNTIME: 'D:\\AICode\\运行数据\\江湖有旅人\\内容生产App\\instance-D',
  TEAMBUILDING_SHARED_MATERIAL_ROOT: 'D:\\AICode\\运行数据\\江湖有旅人\\内容生产App\\shared-material',
  TB_USER_DATA_ROOT: 'D:\\AICode\\运行数据\\江湖有旅人\\内容生产App\\instance-D\\electron-userdata',
  CONTENT_ACCOUNT_IDS: 'account-4',
  CONTENT_ONLY_MODE: '1',
...
function start() {
  console.log('[Instance D Daemon] 正在启动实例 D Electron 进程...');
  ...
  proc.stdout.on('data', d => console.log('[Instance D STDOUT]', d.toString().trim().slice(0, 300)));
  proc.stderr.on('data', d => {
    const s = d.toString().trim();
    if (!s.includes('ExtensionLoadWarning') && !s.includes('deprecated')) {
      console.error('[Instance D STDERR]', s.slice(0, 300));
    }
  });

  proc.on('exit', (c, sig) => {
    console.warn('[Instance D Daemon] 退出 code=' + c + ', sig=' + sig + '，5秒后自动重启...');
