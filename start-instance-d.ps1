...
$env:CONTENT_INSTANCE_ID = "D"
$env:CONTENT_INSTANCE_LABEL = "实例 D · 账号4"
$env:PORT = "4334"
$env:TB_REMOTE_DEBUGGING_PORT = "9434"
$env:TEAMBUILDING_DASHBOARD_RUNTIME = "D:\AICode\运行数据\江湖有旅人\内容生产App\instance-D"
$env:TEAMBUILDING_SHARED_MATERIAL_ROOT = "D:\AICode\运行数据\江湖有旅人\内容生产App\shared-material"
$env:TB_USER_DATA_ROOT = "D:\AICode\运行数据\江湖有旅人\内容生产App\instance-D\electron-userdata"
$env:CONTENT_ACCOUNT_IDS = "account-4"
...
for ($attempt = 0; $attempt -lt 40; $attempt += 1) {
  ...
  try { $ready = $client.ConnectAsync("127.0.0.1", 4334).Wait(300) } catch { $ready = $false } finally { $client.Dispose() }
  if ($ready) { break }
}
if (-not $ready) { throw "内容生产服务未能监听 4334，详见 $serverErr" }
$exitCode = 0
try {
